...
public class NaiveBayesProject {
    ...
    public static void main(String argsp[])
    {
        ...
        try {
          ...
          //Builds the Naive Bayes Model
            ...
            //Arrays.fill(weights, 1);
            weights = HillClimbing(train,weights,.9,.00001);
    ...
    public static double[] HillClimbing(Instances train,double weights[], double learningRate, double AOCCheck) throws Exception
    {
    	...
    	double[] AUCPost = new double[train.numAttributes()];
    	double AUC;
		...
		for(int i =0; i < train_HC.numAttributes(); i++)
		{
    		...
    		while(breaker)
    		{
    			...
    			if((AOCCheck < (AUCPost[i] - AUC) && (AUCPost[i]- AUC) >0)  || AUCPost[i] == 0)
    			{
    				weigh_change[i] = (learningRate*oAOC)*Math.pow((1-oAOC),2);
    				//System.out.println("Current i : "+ i + "Current delta weight :" + weigh_change[i]);
    				weights[i] += weigh_change[i];
    				//Breaks if the weight becomes zero
    				if(weights[i] < 0)
    				{
    					weights[i] = 0;
    					breaker = false;
